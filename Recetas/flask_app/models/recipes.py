...
class Recipe:
    ...
    def save(cls, formulario):
        ...
        result = connectToMySQL('recetas_g2').query_db(query, formulario)
        return result

    @classmethod
    def get_all(cls):
        query = "SELECT recipes.*, first_name  FROM recipes LEFT JOIN users ON users.id = recipes.user_id;"
        results = connectToMySQL('recetas_g2').query_db(query) #Lista de diccionarios 
        recipes = []
    ...
    def get_by_id(cls, formulario): #formulario = {id: 1}
        ...
        result = connectToMySQL('recetas_g2').query_db(query, formulario) #Lista de diccionarios
        recipe = cls(result[0])
        return recipe
